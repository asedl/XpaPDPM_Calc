 ...
 */
public class NRS_ScoringModel_v3312 extends ClinicalFunctional_ScoringModel_v3312 {
	 ...
	 * @param dataManager
	 */
	public NRS_ScoringModel_v3312(HomeHealthGrouperIF grouper,
	...
	public ScoringPointsIF score(HomeHealthRecordIF record,
		...
		if (this.grouper.getListenerCount() > 0) {
			ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this, "Final NRS equation score: ",
			Integer.toString(points.getScores()[0]));
		}
	...
	public void preprocessRecord(HomeHealthRecordIF record,
		...
		// check if the code is a scoring code or a payment code
			...
			if (code.isVCode()) {
				...
				if (code.isUlcer()) {
					...
					ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this,
							"Set M0230_PRIMARY_DIAG_ICD not valid for scoring due to missing Ulcer code in column 3");
				}
				...
				primaryCode.setValidForScoring(false);
				ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this,
						"Set M0230_PRIMARY_DIAG_ICD not valid for scoring due to missing Ulcer code");
		...
		switch (diagCode.getDiagnosticGroup().getId()) {
			...
			default:
				if (getGrouper().getListenerCount() > 0) {
					ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this,
							"Diagnosis code '", diagCode.getCode(), "' valid for scoring, but Diagnosis Group ",
							diagCode.getDiagnosticGroup().getDescription(),
	...
	public String getName() {
		return "Non-Routine Supplies Scoring Model 3312";
	}
