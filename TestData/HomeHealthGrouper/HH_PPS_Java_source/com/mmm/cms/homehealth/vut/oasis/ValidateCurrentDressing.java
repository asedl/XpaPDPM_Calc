 ...
 */
public class ValidateCurrentDressing extends AbstractFunctionalItemValidator implements RecordItemValidatorIF {
    ...
    public int validate(HomeHealthRecordIF record, CollectionValidationEditsIF edits) {
        ...
        if (ValidateUtils.isValidValue(record.getASSMT_REASON(), AbstractItemValidator.ASSESSMENT_1_3_4_5_9_ONLY)) {
            if (!ValidateUtils.isValidValue(record.getCRNT_DRESS_UPPER(), 
                ...
                edits.add(new OasisValidationEdit(OasisEditsEN.EDIT_3060,
                        new HHOasisDataItem(HomeHealthRecord_C1_IF.OASIS_C1_ITEM_M1810_CRNT_DRESS_UPPER, record.getCRNT_DRESS_UPPER())));
            }
            if (!ValidateUtils.isValidValue(record.getCRNT_DRESS_LOWER(), 
                    ValidateUtils.ARRAY_DOUBLE_0, 
                    ValidateUtils.ARRAY_DOUBLE_1, 
                    ValidateUtils.ARRAY_DOUBLE_2, 
                    ValidateUtils.ARRAY_DOUBLE_3)) {
                count++;
                edits.add(new OasisValidationEdit(OasisEditsEN.EDIT_3060,
                        new HHOasisDataItem(HomeHealthRecord_C1_IF.OASIS_C1_ITEM_M1820_CRNT_DRESS_LOWER, record.getCRNT_DRESS_LOWER())));
            }
        }
    ...
    public String getDescription() {
        return "Validates Dress Upper Lower for edits: 3060";
    }
