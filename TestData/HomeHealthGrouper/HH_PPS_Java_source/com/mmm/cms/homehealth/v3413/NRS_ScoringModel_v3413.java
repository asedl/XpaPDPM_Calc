 ...
 */
public class NRS_ScoringModel_v3413 extends ClinicalFunctional_ScoringModel_v3413 {
	 ...
	 * @param dataManager
	 */
	public NRS_ScoringModel_v3413(HomeHealthGrouperIF grouper,
	 ...
	 *
	 * Nov 2012 - Ensure same diabetic ulcer logic applies when the V-Code in
	 * position 1 is an Primary Awarding V-code. In such a case, then the
	...
	public void preprocessRecord(HomeHealthRecordIF record,
		...
		if (primaryCode.isDiabeticUlcer()) {
			...
			if (code.isOptionalPaymentCode()) {
				...
				if (code.isUlcer()) {
					...
					ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this, "Set PRIMARY_DIAG_ICD not valid for scoring due to missing Ulcer code in column 3");
				}
			} else if (!code.isValidForScoring() || !code.isUlcer()) {
				// do not score the primary code because of the
				// missing Ulcer code
				primaryCode.setValidForScoring(false);
				ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this, "Set PRIMARY_DIAG_ICD not valid for scoring due to missing Ulcer code");
		...
		for (int idx = ulcerIndex; idx < 6; idx++) {
			...
			if (code.isDiabeticUlcer()) {
				code.setValidForScoring(false);
				ScoringEventFormatter.fireScoringGeneral(grouper.getEventListenersList(), grouper, this, "Set DIAG code " + code.getCode() + " not valid for scoring due previous Diabetic Ulcer code");
			}
		}
	...
	public String getName() {
		return "Non-Routine Supplies Scoring Model v3413";
	}
